...
DEFAULTS = {
    ...
    "stats": {},
    "resting_player": None,
    "last_match": None,   # ป้องกันเจอคู่ซ้ำทันที
}
...
# -----------------------------
def force_rerun():
    """บังคับรีเฟรชให้ชัวร์ โดยเฉพาะบนมือถือ"""
    try:
        st.rerun()
    except:
        try:
...
def _choose_resting_player(players: List[str]) -> Optional[str]:
    """พักจากคนที่เล่นเยอะสุด (ถ้ามีคนเกินคู่)"""
    if len(players) % 2 == 0:
        return None
...
def process_result(winner_side: str):
    ...
    # ถ้าชนะครบ 2 → ทีมชนะทั้งทีมต้องออก
...
if ss.get("stats"):
    ...
    st.table([
        ...
        for i, (name, data) in enumerate(ordered, start=1)
    ])
